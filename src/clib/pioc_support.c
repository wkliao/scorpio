                        ...
                        int mode)
{
    ...
    */
    ...
    if (file->iotype == PIO_IOTYPE_ADIOS)
    {
        ...
        snprintf(file->filename, len + 3 + 3, "%s.bp", filename);
        ...
        if (file->mode & PIO_NOCLOBBER) /* Check adios file/folder exists */
        {
            ...
            snprintf(filefolder, strlen(file->filename) + 6, "%s.dir", file->filename);
        ...
        else
        {
            ...
            if (ios->union_rank == 0)
            {
                char bpdirname[PIO_MAX_NAME];
                assert(len + 7 <= PIO_MAX_NAME);
                snprintf(bpdirname, PIO_MAX_NAME, "%s.bp.dir", filename);
                struct stat sd;
