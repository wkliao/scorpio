...
static int PIOc_write_decomp_adios(file_desc_t *file, int ioid)
{
	...
	else if (iodesc->maplen == 1)
	{
		...
		else if (type == adios2_type_int8_t) 
        {
...
static int check_need_to_flush(file_desc_t *file, adios_var_desc_t *av)
{
	int ierr = PIO_NOERR;
	adios2_error adiosErr = adios2_error_none;
                                   ...
                                   void *array, void *fillvalue)
{
	...
	if (file->myrank==file->WRITE_FILLVAL_ID) 
	{
		if (fillbuf) {
			...
			char *dummy_fillval = (char*)calloc(av->fillval_size,sizeof(char));
			assert(dummy_fillval!=NULL);;
			memcpy(av->fillval_buffer+(av->fillval_cnt*av->fillval_size),dummy_fillval,av->fillval_size); 
			free(dummy_fillval);
		} 
		(av->fillval_cnt)++;
	}
	...
	ierr = check_need_to_flush(file, av);
	if (ierr!=PIO_NOERR)
	{ 
		return pio_err(NULL, file, PIO_EADIOS2ERR, __FILE__, __LINE__, 
                     ...
                     void *array)
{
    ...
    if (file->iotype == PIO_IOTYPE_ADIOS)
    {
        return pio_err(ios, file, PIO_EADIOSREAD, __FILE__, __LINE__,
          		"Reading variable (%s, varid=%d) from file (%s, ncid=%d)failed . ADIOS currently does not support reading variables", 
